...
while counter < limiter:
    ...
    ### get snmp values if  timeout occurs, raise an error and try again
    while 1:
        try:
            for des,oid in snmp_oids:
                out = int(snmp_extract(snmp_get_oid_v3(snmp_device,snmp_user, oid)))
                print '%s %s' % (des , out)
                diff = old_vlaue.get(des)
                if diff:
                    rtr_info[des]=out - diff
                old_vlaue[des]=out
            break
        except TypeError:
            print 'error'
    ...
    if  rtr_info:
        ...
        Fa4_InOctects.append(rtr_info['ifInOctects_fa4'])
        Fa4_InUcast.append(rtr_info['ifInUcatsPkts_fa4'])
        Fa4_OutOctects.append(rtr_info['ifOutOctects_fa4'])
        Fa4_OutUcast.append(rtr_info['ifOutUcastPkts_fa4'])
